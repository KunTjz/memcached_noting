...
static void conn_shrink(conn *c) {
    ...
    if (c->rsize > READ_BUFFER_HIGHWAT && c->rbytes < DATA_BUFFER_SIZE) {
		...
		// 如果realloc的第二个参数比原来的malloc返回的内存大小还小
		// 据说会返回原指针
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        if (c->rbytes >= c->rsize) {
            ...
            ++num_allocs;
			// 看见没，conn里面的buf是直接realloc而来的，即不是从
			// 内存池中分配的，而是从系统内存中分配的啊，这样
			// 它的内存池管理就简单很多了
        ...
        if (res == -1) {
			// 这里是非阻塞模式，没有判断EINTR
			// EAGIN是没有数据了，不用再读了
			// EWOULDBLOCK是非阻塞模式下用的，不需要重新读(记住它用于非阻塞，不需要重读即可)
			// 如果要用EINTR, 对于EINTR只要简单的重读就行了.
...
void event_handler(const int fd, const short which, void *arg) {
	...
	// 直接把connect传入了，不用保存全局的<fd,connection>信息来查找
	// 不过相信在libevent内部，还是要保存这样的信息，因为epoll始终只能
	// 返回fd
    c = (conn *)arg;
    assert(c != NULL);
...
static int server_socket(const char *interface,
    ...
    for (next= ai; next; next= next->ai_next) {
        ...
        if (IS_UDP(transport)) {
			...
			// 这里注册的READ事件
			// 在conn_new中，会具体注册event_handler函数
			// event_handler中调用核心状态机处理函数，根据connect
			// 的状态来进行不同的处理，而不是采用注册不同处理函数的方法
