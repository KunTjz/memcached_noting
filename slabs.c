...
typedef struct {
	...
	// slots中存储的是还没有用的和回收回来的item，item是在trunk中分配的
    void *slots;           /* list of item ptrs */
    unsigned int sl_curr;   /* total free items in list */ //这是free的item的个数

    unsigned int slabs;     /* how many slabs were allocated for this class */

	// 一个slabclass可以管理多个slab，而slab包含多个trunk
	// list_size是当前已有的slab的数目,他肯定小于等于slabs
    void **slab_list;       /* array of slab pointers */
    unsigned int list_size; /* size of prev array */


    unsigned int killing;  /* index+1 of dying slab, or zero if none */
    size_t requested; /* The number of requested bytes */ // 已经使用内存大小
} slabclass_t;
 ...
 */
// 找到第一个能装下它的slabclass
// 同一个slabclass的slab的大小一样
// 不同slabclass的slab大小倍增
...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    if (prealloc) {
        ...
        // 这里就先把内存全都分配了，使用mem_base, mem_current,
		// mem_avail来管理内存池
	...
	// 初始化所有的slabclass	
	// factor为增长因子，即size * factor不能大于最大的trunk size
    while (++i < POWER_LARGEST && size <= settings.item_size_max / factor) {
        	
		// 设置item是8字节对齐的,这里的操作就是让size为8的倍数
...
// 扩充slab的数量，而slab的大小是固定了的
static int grow_slab_list (const unsigned int id) {
    slabclass_t *p = &slabclass[id];
    if (p->slabs == p->list_size) {
		// 按照2倍的方式扩充slab的数量，最初为16个。类似vector
