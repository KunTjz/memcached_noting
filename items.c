...
item *do_item_alloc(char *key, const size_t nkey, const int flags,
	...
	*/
    ...
    mutex_lock(&cache_lock);
    /* do a quick check if we have any expired（超时） items in the tail.. */
    int tries = 5;
    int tried_alloc = 0;
    item *search;
    void *hold_lock = NULL;
...
void item_free(item *it) {
    ...
    it->slabs_clsid = 0;
    DEBUG_REFCNT(it, 'F');
    // 没有真正free，而是放在了slabclass中的slots里面
...
void do_item_update(item *it) {
    ...
    if (it->time < current_time - ITEM_UPDATE_INTERVAL) {
        ...
        mutex_lock(&cache_lock);
        if ((it->it_flags & ITEM_LINKED) != 0) {
           	// 先从LRU队列中删除，再加入队列中，这样它就成为LRU队列中
			// 的第一个，就是它刚用过的意思
			item_unlink_q(it);
            it->time = current_time;
            item_link_q(it);
...
int do_item_replace(item *it, item *new_it, const uint32_t hv) {
	...
	// 在hash表和LRU队列中删除原来的item，然后加入新的item
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv) {
    ...
    if (it != NULL) {
        ...
        if (settings.oldest_live != 0 && settings.oldest_live <= current_time &&
        ...
        } else if (it->exptime != 0 && it->exptime <= current_time) { // 自己过期了
            do_item_unlink(it, hv);
            do_item_remove(it);
            it = NULL;
            if (was_found) {
            ...
            it->it_flags |= ITEM_FETCHED; // 标记为已读
            DEBUG_REFCNT(it, '+');
        }
    }
