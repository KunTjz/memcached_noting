...
void item_lock(uint32_t hv) {
	...
	// 这里根据锁的类型来锁，如果是GRANULAR,则只锁住
	// 一个桶即可啊
	// 这样操作hash表的时候就不用锁住整个hash表，效率
	// 提高不止一点点啊。  这是C语言自己实现数据结构的
	// 好处！   remember
...
void switch_item_lock_type(enum item_lock_types type) {
	...
	// 这里发送转换锁的消息后，等待worker线程转换
	// 之前已经把init_count设为0了，所以只有当
	// 所有worker线程全部转换完成后才会被唤醒
...
static void setup_thread(LIBEVENT_THREAD *me) {
	...
	// 已经监听了recvfd,所以主线程通过sendfd写数据的时候
	// 能够监听到, 这里注册的函数是libevent_process，不是event_handler
	// 正常连接的socket fd注册的是event_handler,里面调用drive_machine
    /* Listen for notifications from other threads */
    event_set(&me->notify_event, me->notify_receive_fd,
              EV_READ | EV_PERSIST, thread_libevent_process, me);
 	
	// 表明me->notify_event这个事件是要加到me->base里面
	// 看下面调用add的时候，就没有再指明me->base了
...
static void thread_libevent_process(int fd, short which, void *arg) {
    ...
    if (NULL != item) {
		...
		// 这里"释放"了item，课件item是fd和connection中间的过度，accpet返回
		// 连接fd后，用item保存一些信息，放入connection queue item中，
		// 用它来生成connection的。生成后当然不再需要item，所以"释放"它
        cqi_free(item);
    }
        break;
...
void dispatch_conn_new(int sfd, enum conn_states init_state, int event_flags,
    ...
    char buf[1];
	// round-robin算法，用于轮训调度，比我自己写的调度算法
	// 简介多了。  学着点!
	...
	// 这里往send_fd里面写入数据，读的话从recvfd中读
	// 可见它应该是写给工作线程，告诉工作线程有连接了。
    buf[0] = 'c';
