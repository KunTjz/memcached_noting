...
// 应该就是程序的全局状态，所以在修改它的时候需要将它锁住
// 服务器应该提供这样的全局状态信息，可以方便时刻获得服务器
// 的运行状态。 这是个很好的设计
/**
 * Global stats.
 */
...
typedef struct _stritem {
    struct _stritem *next;	// 下一个item 注意这是在LRU队列中用的
    struct _stritem *prev;	// 上一个item 注意这是在LRU队列中用的
    struct _stritem *h_next;    /* hash chain next */ //这是hash链表中的下一个，没有prev
    ...
    // 使用union 是为了在没有使用cas的时候节省定义cas的8个字节的空间
	union {
        uint64_t cas;  // cas: check and set
        char end;
    } data[];
...
typedef struct {
    ...
    cache_t *suffix_cache;      /* suffix(后缀，下标) cache */
